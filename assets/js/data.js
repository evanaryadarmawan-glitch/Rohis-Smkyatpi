...
const ROHIS = {
    ...
    updatePengurus(id, item) {
        const arr = this.getPengurus().map(p => p.id === id ? { ...p, ...item } : p);
        return this.savePengurus(arr);
    },

    /* ---------------- PEMBINA ROHIS ---------------- */
    getPembina() { return this._get(this.KEYS.PEMBINA, []); },
    savePembina(arr) { return this._set(this.KEYS.PEMBINA, arr); },
    addPembina(item) {
        const arr = this.getPembina();
        arr.push({ id: this._id(), ...item });
        return this.savePembina(arr);
    },
    updatePembina(id, item) {
        const arr = this.getPembina().map(p => p.id === id ? { ...p, ...item } : p);
        return this.savePembina(arr);
    },
    deletePembina(id) {
